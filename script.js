function CreazioneElemento(contenuto,div){
    ...
    img_prefer.src = "images1/preferiti-vuota.png";
...
function togliDettagli(event){
    ...
    childNodes[2].remove();                  //rimuove il nodo p contenente la descrizione del prodotto.
    childNodes[2].remove();                  //rimuove il nodo em contenente il prezzo del prodotto.
...
function aggiungiPreferiti(event){
    ...
    if(preferiti.childNodes.length > 1){
        preferiti.childNodes[1].classList.add('hidden');                 //nasconde il testo:"Non hai selezionato nessun elemento".
        /*preferiti.childNodes[1].remove()*/
    }

    img.src = "images1/preferiti-piena.png";
    const new_div = document.createElement('div');
    new_div.innerHTML = div.innerHTML;
    
    new_div.childNodes[0].childNodes[1].src = "images1/preferiti-piena.png";
...
function togliPreferiti(event){
    ...
    for(let i = 0; i< grid_elements.childNodes.length; i++){
        if(grid_elements.childNodes[i].childNodes[0].childNodes[0].textContent === h3.childNodes[0].textContent){
            grid_elements.childNodes[i].childNodes[0].childNodes[1].src = "images1/preferiti-vuota.png";
    ...
    if(grid.childNodes.length === 0){
        preferiti.childNodes[1].classList.remove('hidden');                //riappare il testo: "Non hai selezionato nessun elemento".
...
function ricerca(event){
    ...
    while(grid_ricerca.childNodes.length){
        grid_ricerca.removeChild(grid_ricerca.firstChild);              //ad ogni lettera aggiunta o rimossa vengono eliminati tutti gli elementi
    }                                                                   //presenti nel div con id #risultatiRicerca.
    ...
    if(testo!==''){
        ...
        for(contenuto of contenuti){
            if(contenuto.titolo.toLowerCase().indexOf(testo.toLowerCase())!==-1){
                ...
                div_aux.childNodes[0].childNodes[1].addEventListener('click',aggiungiPreferiti);
                div_aux.childNodes[2].addEventListener('click',aggiungiDettagli);
